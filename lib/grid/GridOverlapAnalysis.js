...
/**
 * 重複ペアの列から、重複でつながったセルのまとまり(連結成分)を求める。
 * Union-Find(素集合)で実装。A-B、B-Cが重複ペアにあれば、AとCが直接重複していなくても
 * 同じクラスタにまとめる(1つ残して他を飛ばす、という統合判断の単位にするため)。
 *
 * @param {Array<{aId:number, bId:number}>} pairs
 * @returns {number[][]} 各要素が1つのクラスタに属するグリッドIDの配列。サイズの降順
 */
function findOverlapClusters(pairs) {
  const parent = {};
  const find = function(x) {
    if (!(x in parent)) { parent[x] = x; return x; }
    let root = x;
  ...
  pairs.forEach(function(p) { union(p.aId, p.bId); });

  const groups = {};
  Object.keys(parent).forEach(function(idStr) {
...
/**
 * クラスタサイズの分布を人が読める1行に整形する(例: "2件クラスタ×5 / 3件クラスタ×2")。
 * @param {number[][]} clusters
 * @returns {string}
 */
function clusterSizeSummary(clusters) {
  const counts = {};
  clusters.forEach(function(c) { counts[c.length] = (counts[c.length] || 0) + 1; });
  return Object.keys(counts)
    .sort(function(a, b) { return Number(a) - Number(b); })
    .map(function(size) { return size + '件クラスタ×' + counts[size]; })
    .join(' / ');
}
