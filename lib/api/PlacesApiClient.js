...
/**
 * Nearby Search (New) が1回のコールで返す最大件数(Places API (New) の仕様、通称「20件の壁」)。
 * API仕様の定数なので、呼び出しを行うこのファイルに置く。
 * この件数に達したかどうかで飽和判定を行う側(lib/crawler/PlaceTypeSetCellSearch.js,
 * lib/crawler/TypeGroupCellSearch.js, entrypoints/surveyAllCells.js)から参照される。
 */
