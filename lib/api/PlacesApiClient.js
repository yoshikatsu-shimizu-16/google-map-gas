...
/**
 * searchNearby が1回のリクエストで返す最大件数(= リクエストの maxResultCount)。
 *
 * Nearby Search (New) は pageToken を非サポートなので、この件数を超える分は
 * 原理的に取得できない。これが「20件の壁」で、ちょうどこの件数返ってきたら
 * 切り捨てが起きている疑いがある = 飽和、と判定する。
 *
 * リクエストに載せる値と飽和の判定値は必ず同じでなければならない。別々に持って
 * ずれると、「永久に分割し続ける」(判定値のほうが小さい)か「飽和を見逃す」
 * (判定値のほうが大きい)かのどちらかが起きる。API仕様の定数なので、
 * INCLUDED_TYPES_MAX_PER_REQUEST と同じく呼び出しを行うこのファイルに置き、
 * 判定する側はここを参照する。
 */
...
function callSearchNearby(apiKey, fieldMask, includedTypes, lat, lng, radius) {
  ...
  const payload = {
    ...
    maxResultCount: MAX_RESULT_COUNT, // 1回あたりの最大取得件数(この値が「20件の壁」)
    locationRestriction: {
      circle: {
