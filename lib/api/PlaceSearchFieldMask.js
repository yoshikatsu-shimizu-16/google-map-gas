 ...
 * ===== 調査用のフィールド指定(Pro段のみ) =====
 *
 * 「どう割れば20件に収まるか」を調べるだけなら rating も websiteUri も要らない。
 * Pro段に収めれば無料枠が 1,000 から 5,000 に増えるため、試行錯誤はこちらで行い、
 * Enterprise枠は答えが出たあとの収穫だけに使う。
 *
 * primaryTypeDisplayName(日本語のラベル)はどの段に属するか未確認のため入れていない。
 * 調査で必要なのは機械可読な types であって表示名ではない。
 */
const PLACE_SURVEY_FIELD_MASK = [
  'places.id',
  'places.displayName',
  'places.primaryType',
  'places.types',
...
/**
 * fieldMask から課金SKUを判定する。SKUは「どのフィールドを要求したか」で決まるため、
 * 呼び出し側にSKUを指定させず、マスクから導出する(二重管理を避ける)。
 *
 * 未知のマスクは Enterprise 扱いにする。取り違えたときに小さいほうの枠で止まるほうが安全で、
 * 逆だと無料枠を超えて課金が発生しうるため。
 *
 * SKUの識別子そのものは lib/api/PlacesApiClient.js が持つ(枠の値と同じ場所にあるべきなので)。
 * ここでは関数の中からのみ参照しており、GASのファイル評価順に依存しない。
 *
 * @param {string} fieldMask
 * @returns {string} API_SKU_PRO または API_SKU_ENTERPRISE
 */
function apiSkuOfFieldMask(fieldMask) {
  return fieldMask === PLACE_SURVEY_FIELD_MASK ? API_SKU_PRO : API_SKU_ENTERPRISE;
}
