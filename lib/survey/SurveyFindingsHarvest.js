 ...
 *
 * ただし「0件」の適用だけは、座標一致に加えて半径も確認する。旧運用の半径700mで
 * 0件だったセルを、その後 fixUnprocessedRootRadius で717mに広げた「グリッド一覧」に
 * そのまま適用すると、広がった分の環状部分(700〜717m)を一度も検索せずに完了扱いに
 * してしまい、その範囲の店を永久に見逃す。飽和(20件以上)の適用にはこの確認が要らない
 * ―― 調査時の半径が現在より小さくても、狭い範囲で20件出ているなら広い範囲でも
 * 20件以上のままのはずで、逆に調査時の半径が現在より大きくても、子は現在のセル自身の
 * 大きさから生成するため取りこぼしは起きない。
 */
 ...
 * @returns {Map<number, {found: number, lat: number, lng: number, radius: number}>}
 */
function readSurveyCellFindings(surveyCellSheet) {
  ...
  surveyCellSheet.getRange(2, 1, lastRow - 1, AREA_SURVEY_CELL_HEADERS.length).getValues()
    .forEach(function(row) {
      ...
      findings.set(gridId, { found: found, lat: row[latIdx], lng: row[lngIdx], radius: row[radiusIdx] });
    });
  return findings;
}
 ...
 * 「0件」の適用はさらに、調査時の半径が現在の半径以上であることも確認する
 * (ファイル先頭のコメントを参照)。
 *
 ...
 *   その場でステータス列を更新する)
 * @param {Map<number, {found: number, lat: number, lng: number, radius: number}>} surveyFindings -
 ...
 *   shapeMismatchSkipped: number, emptyCoverageGrewSkipped: number}}
 */
function applySurveyFindingsToUnprocessedGrids(gridSheet, gridValues, surveyFindings, nextGridId) {
  ...
  let emptyCoverageGrewSkipped = 0;
  ...
  for (let i = 0; i < gridValues.length; i++) {
    ...
    if (finding.found === 0) {
      const currentRadius = row[3];
      if (finding.radius < currentRadius) {
        // 調査時より現在の検索範囲が広い(半径修正などで拡張された)。広がった分の
        // 環状部分は調査していないため、0件だったという答えを信用できない。
        emptyCoverageGrewSkipped++;
        continue;
      }
  ...
  return {
    ...
    emptyCoverageGrewSkipped: emptyCoverageGrewSkipped
  };
}
