 ...
 *
 * ----- 役割の変更(SEARCH_STRATEGY=probe 導入後) -----
 * SEARCH_STRATEGY=type_groups(既定)ではこの4グループが「毎セル無条件に検索する単位」
 * だったが、SEARCH_STRATEGY=probe では「プローブ集合(PLACE_TYPE_PROBE_SET)が20件に
 * 飽和したセルだけを網羅するためのフォールバック手段」に役割が変わる
 * (lib/crawler/ProbeFirstCellSearch.js / lib/crawler/TypeGroupCellSearch.js を参照)。
