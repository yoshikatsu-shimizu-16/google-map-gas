 ...
 *
 * ----- 役割の変更(2026-09-20、通常経路をプローブ集合1コールへ一本化) -----
 * かつてはこの4グループが「毎セル無条件に検索する単位」だったが、通常経路は
 * プレイスタイプ集合(PLACE_TYPE_SEARCH_SET)1コールに一本化し、20件飽和セルは空間分割
 * (四分木分割)に回すようにした(lib/crawler/PlaceTypeSetCellSearch.js)。
 * この4グループを使った探索(lib/crawler/TypeGroupCellSearch.js)は通常経路からは
 * 呼ばれなくなったが、削除はしていない。空間分割でも解決しない飽和マス向けの
 * タイプ分割として、次のステップで再利用する想定(docs/survey-next-actions-2026-09-20.md)。
 * 4グループそのものの定義・分割ロジックは変更していない。
 */
const TYPE_GROUP_A = [
 ...
 * API制約があるため、プレイスタイプ集合(lib/catalog/PlaceTypeSearchSet.js)や被覆分析
