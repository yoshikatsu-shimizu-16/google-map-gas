...
| `crawlAllGrids` | `entrypoints/crawlAllGrids.js` | トリガー対象(手動再実行も可) | グリッド巡回・店舗検索・「全飲食店データ」への書き込み | 日次3時台の自動トリガー対象。関数名は変更禁止(トリガーが文字列で参照)。1セル1コール(プローブ集合)、飽和したら4分割。**LockServiceで排他制御**しており、他の実行が進行中なら待たずに即終了する(無駄な重複コールの防止)。実行のたびに「調査(マス)」の既知の結果を未処理行に適用し、答えが分かっているコールを省く(下記) |
...
| I | エラー回数 | 恒久エラーの再試行を打ち切るようにした際(Issue #37) |

「エラー回数」は同じセルが連続で検索に失敗した回数です。`エラー` は完了扱いではないので
次回の実行で自動的に再試行されますが、`MAX_GRID_ERROR_RETRIES`(3回)連続で失敗すると
`要確認(エラー継続)` に倒れ、自動での再試行を打ち切ります
(`lib/crawler/GridErrorRetryPolicy.js`)。打ち切らないと、恒久的に失敗する行を日次トリガーの
たびに叩き続けることになり、**クォータ起因でないエラーは自前カウンタから返却されない**ため
壊れた行1つにつき毎日1コールずつ無料枠が減ります。原因を直したあと再開したい場合は、
その行の「処理状況」を `未処理` に、「エラー回数」を `0` に戻してください。
...
#### 調査結果の適用(答えが分かっているコールを省く、Issue #39)

`crawlAllGrids` は `ensureGridSchemaMigrated` の直後、本体のループに入る前に
「調査(マス)」(`surveyAllCells` がPro段で調べた既知の結果)を読み、`未処理`の行に
次の判断を適用します(`lib/survey/SurveyFindingsHarvest.js`)。

- **0件だったセル** → `処理済み(0件)` にしてコールしない
- **飽和(20件以上)だったセル** → 親のコールを省き、いきなり子を4分割生成する
  (`密集(分割済み)`。ただし既に `MAX_TIER` の場合は子を作らず `要確認(上限到達)`)
- **1〜19件だったセル・調査に無いセル** → 変更しない。Pro段の調査には評価・HPなど
  営業データが無いため、この情報だけでは省略できず通常どおり1コールで収穫する

適用は「グリッド一覧」側の中心座標と調査記録の中心座標が一致する行だけに限ります。
同じグリッドIDでも座標が食い違う場合は「生成条件が変わった可能性がある行」として
参考情報にとどめ、通常どおり実際に検索して確かめます(半径ではなく座標で比較するのは、
座標は生成時に決め打ちで固定される一方、半径は計算式の変遷により旧データと食い違う
ことが実際にあったため。`docs/survey-next-actions-2026-09-20.md` 2-3節)。

**「0件」の適用だけは、座標一致に加えて調査時の半径が現在の半径以上であることも
確認します。** 旧運用の半径700mで0件だったセルを、`fixUnprocessedRootRadius` で
717mに広げた後の行にそのまま適用すると、広がった分の環状部分(700〜717m)を
一度も検索せずに完了扱いにしてしまい、その範囲の店を永久に見逃すためです。
飽和(20件以上)の適用にはこの確認は不要です(狭い範囲で20件出ていれば広い範囲でも
20件以上のままで、逆に子は現在のセル自身の大きさから生成するため取りこぼしません)。

「調査(マス)」シートが無い場合(未実施)は何もせず、従来どおり全セルを通常経路で処理します。
`CRAWL_MAX_CALLS=0`(試算モード)のときも、シートを書き換えないという既存の約束を
守るため、この適用自体を行いません。
